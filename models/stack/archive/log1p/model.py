...
class RadarEncoder(nn.Module):
    ...
    3-block CNN with adaptive pooling to support any spatial patch size.
    """

    def __init__(self, in_channels=73, latent_dim=512, dropout_rate=0.25):
        super().__init__()

        self.conv1 = nn.Conv2d(in_channels, 192, kernel_size=3, padding=1)
...
class ScalarDecoder(nn.Module):
    ...
    All model capacity focuses on predicting the gauge value directly.
    """
...
class PrecipitationStackModel(nn.Module):
    ...
    def __init__(self, latent_dim=512, add_bias=False, dropout_rate=0.25, output_size=9, scalar_output=False):
        super().__init__()
