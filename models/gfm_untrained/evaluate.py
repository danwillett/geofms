"""
evaluate.py — Evaluate a trained from-scratch (untrained) GFM.

Mirrors models/gfm/evaluate.py but for the self-contained UntrainedGFM module:
no upscaling, native-tile inference, center-pixel extraction. Computes both naive
expm1 and Jensen-corrected metrics for log-space runs, and runs the daily test.

Run from the project root:
    python -m models.gfm_untrained.evaluate --checkpoint-dir models/checkpoints/gfm_untrained
"""
...
from models.gfm_untrained.model import UntrainedGFM
from models.gfm_untrained.dataset import RadarDEMDataModule, create_heavy_rain_sampler


# ── DEFAULT CONFIG ──────────────────────────────────────────────────────────────
DEFAULT_PICKLE   = "dataset/outputs/3d/radar_gauge_dataset_subml_daygroup_offsets_9500.pkl"
DEFAULT_CKPT_DIR = "models/checkpoints/gfm_untrained"
DEFAULT_OUTPUT   = "evaluation_figures/gfm_untrained"
# ───────────────────────────────────────────────────────────────────────────────


def find_checkpoint(checkpoint_path=None, checkpoint_dir=None):
...
def load_model(checkpoint_path):
    ...
    model = UntrainedGFM.load_from_checkpoint(checkpoint_path)
    model.eval()
    model.to(device)
...
def _center_predict(model, batch, device):
    """Return (pred_center, target_center) at the central (gauge) pixel.

    The validation/test datasets use augment=False, so the gauge sits at the
    tile center (out//2). No upscaling — the model consumes the native tile.
    """
    ...
    center = out // 2
    pred_center   = pred[:, center, center].detach().cpu().numpy()
...
def estimate_jensen_correction(pred_log, target_log):
    ...
    return 0.5 * float(np.var(target_log - pred_log))
...
def run_inference(model, datamodule, device):
...
def print_report(preds_mm, targets_mm, metrics, metrics_jensen=None, jensen_c=None):
    print(f"\n{'='*60}")
    print(f"📊 GFM-UNTRAINED EVALUATION")
    print(f"{'='*60}")
...
def plot_evaluation(preds_mm, targets_mm, metrics, output_dir):
    out = Path(output_dir)
    ...
    ax.set_xlabel("Actual (mm/hr)"); ax.set_ylabel("Predicted (mm/hr)")
    ax.set_title(f"Predicted vs Actual\nR²={metrics['r2']:.3f}, MAE={metrics['mae']:.3f} mm/hr")
    ax.set_xlim(0, max_val); ax.set_ylim(0, max_val); ax.legend(); ax.grid(True, alpha=0.3)
    ...
    for lo, hi in zip(bounds, bounds[1:]):
        ...
        if m.sum():
            ...
            maes.append(np.mean(np.abs(err))); biases.append(np.mean(err)); counts.append(m.sum())
        else:
    ...
    ax.set_ylabel("MAE (mm/hr)"); ax.set_title("Error by Precipitation Category")
    ...
    ax.set_xlabel("Predicted (mm/hr)"); ax.set_ylabel("Residual (Actual − Predicted)")
    ax.set_title("Residual Plot\n(above zero = underpredicting)"); ax.grid(True, alpha=0.3)
    ...
    ax.set_xlabel("Precipitation (mm/hr)"); ax.set_ylabel("Density")
    ax.set_title("Precipitation Distribution"); ax.legend()
    ax.set_xlim(0, max(targets_mm.max(), preds_mm.max())); ax.grid(True, alpha=0.3, axis="y")

    plt.tight_layout()
    save_path = out / "gfm_untrained_evaluation.png"
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    print(f"✓ Saved evaluation plot to: {save_path}")
    plt.close()


def plot_jensen_comparison(targets, preds_naive, preds_jensen, metrics_naive,
                           metrics_jensen, jensen_c, output_dir, filename, unit="mm/hr"):
    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)
...
def evaluate(
    ...
    if log_target and metrics_jensen is not None:
        plot_jensen_comparison(targets_mm, preds_mm, preds_mm_jensen,
                               ...
                               fig_dir, "gfm_untrained_jensen_comparison.png", unit="mm/hr")
    ...
    test_summary = evaluate_test(
        ...
        modality_layout=modality_layout, use_dem=use_dem, jensen_c=jensen_c)

    # ── Persist metrics into the run's config.json (so each run keeps its own
    #    val + test numbers; the sweep summary otherwise gets overwritten). ──
    val_summary = {
    ...
    _persist_metrics(run_dir if run_dir else str(Path(ckpt).parent),
                     val_summary, test_summary)

    return metrics, test_summary, (run_dir if run_dir else str(Path(ckpt).parent))
...
def evaluate_test(model, device, pickle_path, output_dir,
                  output_size=18, log_target=True, fields=None, use_feature_masks=True,
                  modality_layout='grouped', use_dem=True, jensen_c=0.0):
    """Evaluate on the daily cumulative gauge test set (spatially disjoint stations).
    ...
    from models.gfm_untrained.dataset import RadarDEMDataset, PICKLE_FIELD_ORDER
    ...
    hourly_preds, hourly_preds_jensen, hourly_meta = [], [], []
    sample_idx = 0
    with torch.no_grad():
    ...
    for pred, pred_j, m in zip(hourly_preds, hourly_preds_jensen, hourly_meta):
        key = (m['date'], m['station_id'])
        daily_groups[key]['pred_sum'] += pred
        daily_groups[key]['pred_jensen_sum'] += pred_j
        daily_groups[key]['count'] += 1
        daily_groups[key]['actual'] = m['daily_precip_mm']
        daily_groups[key]['station_name'] = m['station_name']
    ...
    if test_metrics_jensen is not None:
        plot_jensen_comparison(actual_daily, pred_daily, pred_daily_jensen,
                               ...
                               output_dir, "test_daily_jensen_comparison.png", unit="mm/day")
    ...
    ax.plot([0, max_val], [0, max_val], 'r--', lw=2, label='Perfect')
    ax.set_xlabel('Actual Daily Rainfall (mm/day)'); ax.set_ylabel('Predicted Daily Rainfall (mm/day)')
    ax.set_title(f"Daily Test — Predicted vs Actual\nR²={test_metrics['r2']:.3f}, MAE={test_metrics['mae']:.3f} mm/day")
    ax.set_xlim(0, max_val * 1.05); ax.set_ylim(0, max_val * 1.05); ax.legend(); ax.grid(True, alpha=0.3)
    ...
    for station in unique_stations:
        ...
        station_bias.append(np.mean(pred_daily[mask] - actual_daily[mask]))
    ...
    ax.set_xticks(x_pos); ax.set_xticklabels(station_short, rotation=45, ha='right', fontsize=9)
    ax.set_ylabel('Mean Bias (pred - actual, mm/day)'); ax.set_title('Daily Test — Per-Station Bias')
    ax.grid(True, alpha=0.3, axis='y')
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Evaluate from-scratch GFM")
    p.add_argument("--checkpoint",     default=None)
    p.add_argument("--checkpoint-dir", default=DEFAULT_CKPT_DIR)
    p.add_argument("--pickle",         default=DEFAULT_PICKLE)
    p.add_argument("--output-dir",     default=DEFAULT_OUTPUT)
    p.add_argument("--output-size",    type=int, default=18)
    p.add_argument("--layout",         default='grouped', choices=['single', 'grouped'])
    p.add_argument("--no-dem",         action="store_true", help="Model was trained without the DEM modality")
