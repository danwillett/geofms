"""
train.py — Train the 3D Stack CNN precipitation model.

Run from the project root:
    python -m models.stack_3d.train

Or with custom args:
    python -m models.stack_3d.train --pickle dataset/outputs/3d/radar_gauge_dataset_3d_4500.pkl --batch-size 8 --spatial-head
"""
...
from models.stack_3d.model import PrecipitationStack3DModel, init_weights
from models.stack_3d.dataset import RadarGauge3DDataset, resolve_fields, compute_n_input_channels

# ── DEFAULT CONFIG ────────────────────────────────────────────────────────────
DEFAULT_CONFIG = {
    'pickle_path':    'dataset/outputs/3d/radar_gauge_dataset_3d_4500.pkl',
    'dem_path':       'dem/preserve_dem_10m_utm.tif',
    'checkpoint_dir': 'models/checkpoints/stack_3d_dualpol',
    'lr':             5e-5,
    'weight_decay':   1e-4,
    'max_epochs':     100,
    'batch_size':     8,
    ...
    'n_encoder_blocks': 3,
    'fields':         'dualpol',
    'output_dir':     'evaluation_figures/stack_3d_dualpol',
    'zarr_cache_mb':  1024,
}
...
def _sample_max_dbz(sample):
    if 'max_reflectivity_dbz' in sample:
        return sample['max_reflectivity_dbz']
    if 'radar_patch' in sample:
        return float(np.nanmax(sample['radar_patch'][:, 0, :, :]))
    return float('nan')
...
def save_config(run_dir, cfg, n_params, train_samples, val_samples, model):
    ...
    for name, module in encoder.named_modules():
        if isinstance(module, (nn.Conv2d, nn.Conv3d)) and 'conv' in name:
    ...
    config_data = {
        ...
        'spatial_head': cfg.get('spatial_head', True),
        'model_type': 'stack_3d',
...
def train(cfg: dict = None, run_name: str = None):
    ...
    model = PrecipitationStack3DModel(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train 3D Stack CNN precipitation model")
    parser.add_argument('--no-spatial-head', action='store_true',
                        help='Use MLP decoder instead of 3D spatial conv head')
    ...
    if args.no_spatial_head:
        cfg['spatial_head'] = False
